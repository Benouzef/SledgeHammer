...
import { takeSnapshot, dirs } from "../react-native-view-shot-master";
...
class SignaturePad extends Component {
  ...
  constructor(props) {
    ...
    var generatedHtml = htmlContent(injectedJavaScript);
    console.log(generatedHtml);
    this.source = {html: generatedHtml}; // {html}; //We don't use WebView's injectedJavaScript because on Android, the WebView re-injects the JavaScript upon every url change. Given that we use url changes to communicate signature changes to the React Native app, the JS is re-injected every time a stroke is drawn.
  }
  ...
  _parseMessageFromWebViewNavigationChange = (newUrl) => {
    ...
    if(!this._attemptToExecuteNativeFunctionFromWebViewMessage(parameters)) {
      console.log({parameters, hashUrl}, 'Received an unknown set of parameters from WebView');
    }
  };
  ...
  _bridged_jsError = (args) => {
    ...
    this.props.onError({details: uri});
  };
    ...
    return (
        <View ref='full' style={this.props.style} collapsable={false}>
          <WebView automaticallyAdjustContentInsets={false}
                   ...
                   source={require('./injectedHtml/testBenoit.html')} //{this.source}//{{uri: 'https://github.com/facebook/react-native'}}
